...
export class BlackjackGame {
    ...
    public async playDealerHand() {
        ...
        if (dealer.isBusted) return console.log("Dealer has Too Many...");
        if (dealer.isBlackJack) return console.log("Dealer has a Blackjack...");

        if (dealer.total > 17 || (!GameConfig.IS_DEALER_HIT_ON_17 && dealer.total == 17 && dealer.isSoft()))  {
            if (GameConfig.DEBUG_MODE) console.log("Dealer stands on "+ dealer.getHandTotal() + "...");
            dealer.stand();
            return;
        }

        if (GameConfig.DEBUG_MODE) console.log("Dealer hits on "+ dealer.getHandTotal() +"...");
